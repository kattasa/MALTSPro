...
def make_data(dataset_iteration):
    ...
    while sum(y0_param <= 0) != 0 and sum(y1_param <= 0) != 0:
        ...
        y0_param = np.abs(X[:, 1] + X[:, 2] + error_y)
        y1_param = np.abs(X[:, 1] + 10 * X[:, 2] + error_y)
    ...
    pd.DataFrame(y_unobs).to_csv('././experiments/cont_features/dataset_' + str(seed) + '/Y_unobs.csv', index = False)
    np.random.seed(999)
