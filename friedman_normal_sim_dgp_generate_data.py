...
def make_data(dataset_iteration):
    ...
    y0 = np.array([y0_param[i] + truncnorm.rvs(a = -3, b = 3, size = n_obs_y) for i in range(n_units)])
    y1 = np.array([y1_param[i] + truncnorm.rvs(a = -3, b = 3, size = n_obs_y) for i in range(n_units)])
    print('got data')
...
if __name__ == '__main__':
    dataset_directory = './experiments/friedman_normal_sim_dgp'
